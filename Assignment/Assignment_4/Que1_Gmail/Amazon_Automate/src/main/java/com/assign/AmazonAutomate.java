...
public class AmazonAutomate {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		By freshBtn = By.cssSelector(
				"div.f3-cgi-flyout-section.f3-cgi-flyout-section-stores a  div.f3-cgi-flyout-store-box.f3-cgi-flyout-store-box-left");
		...
		action.moveToElement(skipInput).click().build().perform();
		...
		By Juicedrink = By.cssSelector("[alt=\"Juices&Colddrinks\"]");
		WebElement juiceInput = driver.findElement(Juicedrink);

		action = new Actions(driver).scrollToElement(juiceInput).pause(Duration.ofSeconds(3)).scrollByAmount(0, 200);
		action.build().perform();
		Thread.sleep(Duration.ofSeconds(3));

		Thread.sleep(5000);
		System.out.println("Successfully  Scroll down to Juice and Drink  ! !! !!!");
		driver.quit();
